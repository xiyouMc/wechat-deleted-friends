...
QRImagePath = os.getcwd() + '/qrcode' + str(time.time()) + '.jpg'
...
class MyRequestHandler(BaseHTTPRequestHandler):
    ...
    def process(self,type):
        ...
        if '?' in self.path:
            ...
            # ImageScale(QRImagePath)
            # main()
            #指定返回编码
            enc="UTF-8"  
            # file_object = open(QRImagePath)
            # try:
...
def showQRImage():
    ...
    print QRImagePath + '请使用微信扫描二维码以登录'
